...
export type PublicStoreWorkItem = {
  id: string;
  week: number;
  date: string | null;
  title: string;
...
export function PublicStoreWorkList({ items, contractWeeks }: { items: PublicStoreWorkItem[]; contractWeeks: number }) {
  const [openedId, setOpenedId] = useState("");
  const weeks = Array.from({ length: Math.max(1, contractWeeks) }, (_, index) => index + 1);

  if (!items.length) return <p className="public-empty">아직 공개할 업무가 없습니다.</p>;
              ...
              return (
                <article className="public-work-entry" key={item.id}>
                  <button className="public-work-row" onClick={() => setOpenedId(opened ? "" : item.id)} type="button">
                    ...
                    <em className={written ? "done" : "pending"}>{written ? "기입완료" : "진행 예정"}</em>
                  </button>
                  {opened && (
                    <div className="public-work-detail">
                      ...
                      {item.evidenceUrls.length > 0 && (
                        <ul>
                          {item.evidenceUrls.map((url, index) => <li key={`${url}-${index}`}><a href={url} rel="noreferrer" target="_blank">첨부 파일 {index + 1} 열기</a></li>)}
                        </ul>
                      )}
                    </div>
                  )}
